...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, stats := range allStats.Nodes {
		...
		for breaker, bstats := range stats.Breakers {
			e.gaugeVecs["breakers_estimated_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, breaker).Add(float64(bstats.EstimatedSize))
			e.gaugeVecs["breakers_limit_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, breaker).Add(float64(bstats.LimitSize))
			e.gaugeVecs["breakers_tripped"].WithLabelValues(allStats.ClusterName, stats.Host, breaker).Add(float64(bstats.Tripped))
		}
		...
		for pool, pstats := range stats.ThreadPool {
			...
			e.gaugeVecs["thread_pool_active_count"].WithLabelValues(allStats.ClusterName, stats.Host, pool).Add(float64(pstats.Active))
			e.gaugeVecs["thread_pool_threads_count"].WithLabelValues(allStats.ClusterName, stats.Host, pool).Add(float64(pstats.Threads))
			e.gaugeVecs["thread_pool_largest_count"].WithLabelValues(allStats.ClusterName, stats.Host, pool).Add(float64(pstats.Largest))
			e.gaugeVecs["thread_pool_queue_count"].WithLabelValues(allStats.ClusterName, stats.Host, pool).Add(float64(pstats.Queue))
		}

		// JVM Memory Stats
		e.gaugeVecs["jvm_memory_committed_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, "heap").Add(float64(stats.JVM.Mem.HeapCommitted))
		e.gaugeVecs["jvm_memory_used_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, "heap").Add(float64(stats.JVM.Mem.HeapUsed))
		e.gaugeVecs["jvm_memory_max_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, "heap").Add(float64(stats.JVM.Mem.HeapMax))
		e.gaugeVecs["jvm_memory_committed_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, "non-heap").Add(float64(stats.JVM.Mem.NonHeapCommitted))
		e.gaugeVecs["jvm_memory_used_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, "non-heap").Add(float64(stats.JVM.Mem.NonHeapUsed))

		// Indices Stats
		e.gauges["indices_fielddata_memory_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.FieldData.MemorySize))
		...
		e.gauges["indices_filter_cache_memory_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.FilterCache.MemorySize))
		...
		e.gauges["indices_query_cache_memory_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.QueryCache.MemorySize))
		...
		e.gauges["indices_request_cache_memory_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.RequestCache.MemorySize))
		e.counters["indices_request_cache_evictions"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.RequestCache.Evictions))

		e.counters["indices_translog_operations"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Translog.Operations))
		e.counters["indices_translog_size_in_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Translog.Size))

		e.counters["indices_get_time_ms"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Get.Time))
		e.counters["indices_get_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Get.Total))

		e.counters["indices_get_missing_time_ms"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Get.MissingTime))
		e.counters["indices_get_missing_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Get.MissingTotal))

		e.counters["indices_get_exists_time_ms"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Get.ExistsTime))
		e.counters["indices_get_exists_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Get.ExistsTotal))

		e.counters["indices_search_query_time_ms"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.QueryTime))
		e.counters["indices_search_query_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.QueryTotal))

		e.counters["indices_search_fetch_time_ms"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.FetchTime))
		e.counters["indices_search_fetch_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.FetchTotal))

		e.gauges["indices_docs"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Docs.Count))
		e.gauges["indices_docs_deleted"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Docs.Deleted))

		e.gauges["indices_segments_memory_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Segments.Memory))
		e.gauges["indices_segments_count"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Segments.Count))

		e.gauges["indices_store_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Store.Size))
		...
		e.counters["indices_indexing_delete_time_ms_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Indexing.DeleteTime))
		e.counters["indices_indexing_delete_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Indexing.DeleteTotal))
		...
		e.counters["indices_merges_docs_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Merges.TotalDocs))

		e.counters["indices_refresh_time_ms_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Refresh.TotalTime))
		e.counters["indices_refresh_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Refresh.Total))

		e.counters["indices_search_query_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.QueryTotal))
		e.counters["indices_search_query_time_ms_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.QueryTime))

		e.counters["indices_search_fetch_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.FetchTotal))
		e.counters["indices_search_fetch_time_ms_total"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Indices.Search.FetchTime))
		...
		// Process Stats
		e.gauges["process_cpu_percent"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Process.CPU.Percent))
		e.gauges["process_mem_resident_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Process.Memory.Resident))
		e.gauges["process_mem_share_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Process.Memory.Share))
		e.gauges["process_mem_virtual_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Process.Memory.TotalVirtual))
		e.gauges["process_open_files_count"].WithLabelValues(allStats.ClusterName, stats.Host).Add(float64(stats.Process.OpenFD))
		...
		for _, fsStats := range stats.FS.Data {
			e.gaugeVecs["filesystem_data_available_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, fsStats.Mount, fsStats.Path).Add(float64(fsStats.Available))
			e.gaugeVecs["filesystem_data_free_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, fsStats.Mount, fsStats.Path).Add(float64(fsStats.Free))
			e.gaugeVecs["filesystem_data_size_bytes"].WithLabelValues(allStats.ClusterName, stats.Host, fsStats.Mount, fsStats.Path).Add(float64(fsStats.Total))
			used_percent := uint64(100 * ((float64(fsStats.Total) - float64(fsStats.Free)) / float64(fsStats.Total)))
			free_percent := 100 - used_percent
			e.gaugeVecs["filesystem_data_used_percent"].WithLabelValues(allStats.ClusterName, stats.Host, fsStats.Mount, fsStats.Path).Add(float64(used_percent))
			e.gaugeVecs["filesystem_data_free_percent"].WithLabelValues(allStats.ClusterName, stats.Host, fsStats.Mount, fsStats.Path).Add(float64(free_percent))
		}
	}
